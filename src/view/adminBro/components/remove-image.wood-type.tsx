...
const ShowImage: React.FC<BasePropertyProps> = (props) => {
  ...
  return (
    <Box variant="gray">
      <Label size="lg">Imagem do acabamento</Label>
      {images && images.url ? (
        <ImageList>
          <ImageCard>
            <DropZoneItem
              src={`${images.url}`}
              filename={`Imagem do acabamento - ${record.params.name}`}
        ...
        <MessageBox
          message="Nenhuma imagem cadastrada para esse acabamento até o momento."
          variant="info"
        />
      )}
    </Box>
